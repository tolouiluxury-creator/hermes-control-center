 ...
 * why — a bare "Fehler" would send the user hunting.
 */
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center gap-1.5 p-2 text-center">
        ...
        <p className="text-xs text-[var(--color-ink-muted)]">
          {unauthorized
            ? 'Hermes hat die Anfrage abgelehnt — fehlt der API-Schlüssel?'
            : unreachable
              ? 'Hermes ist gerade nicht erreichbar.'
              : error.message}
        </p>
      </div>
    );
